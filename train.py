...
# size of token sequences to train on (and network size!)
config.window_len = 30
config.vocab_size = 500
config.token_dims = 7  # number of features per token, including token hash
config.positive_fraction = 0.5
# target match scores larger than this will becomes positive labels
config.target_thresh = 0.9
config.epochs = 50
config.batch_size = 10000
config.steps_per_epoch = 10
# how many documents to check extraction on after each epoch
config.doc_acc_sample_size = 25
...
def input_docs2(conn, max_docs=None):
    token_dataset = source.dataset(conn)
    ...
    for token_row in token_dataset:
        # throw out tokens that are too short, they won't help us
        dc_slug, token, x0, y0, page, committee, gross_amount_usd = token_row
        row = {'token': token, 'x0': x0, 'y0': y0, 'slug': dc_slug}
        print(row)
        if len(token.numpy()) < 3:
            continue

        if dc_slug != active_slug:
            if active_slug:
                yield doc_rows
                num_docs += 1
                if max_docs and num_docs >= max_docs:
...
def load_training_data(config, conn):
    slugs = []
    token_text = []
    features = []
    labels = []
    for doc_tokens in input_docs2(conn, max_docs=config.read_docs):
        ...
        labels.append([(0 if float(row['gross_amount']) <
                        config.target_thresh else 1) for row in doc_tokens])
...
if __name__ == '__main__':
    # ALSO TODO: Actual DB credential mgmt
    conn = source.connection("root", "changeme")

    print('Configuration:')
    print(config)

    # slugs, token_text, features, labels = load_training_data(config)
    slugs, token_text, features, labels = load_training_data(config, conn)
