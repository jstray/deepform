...
parser.add_argument("--penalize_missed", type=float, default=5, help="penalize missed")

parser.add_argument(
    "--read_docs", type=int, default=10000, help="how many docs to load, at most"
)
...
parser.add_argument(
    ...
    type=int,
    default=30,
    help="size of token sequences to train on (and network size!)",
)

parser.add_argument(
    "--vocab_size", type=int, default=500, help="size of vocab in the embedding layer"
)
...
parser.add_argument(
    ...
    type=int,
    default=5,
    help="number of features per token, including token hash",
)

parser.add_argument(
    "--positive_fraction", type=float, default=0.3, help="idk what this does"
)
...
parser.add_argument(
    ...
    type=float,
    default=0.9,
    help="target match scores larger than this will becomes positive labels",
)
...
parser.add_argument(
    ...
    type=int,
    default=25,
    help="How many docs to check extraction on after each epoch",
)
...
def load_training_data(config):
    if os.path.isfile("data/cached_features.p"):
        ...
        slugs, token_text, features, labels = pickle.load(
            open("data/cached_features.p", "rb")
        )
    else:
        ...
        pickle.dump(
            (slugs, token_text, features, labels), open("data/cached_features.p", "wb")
        )
...
def predict_scores(model, features, window_len):
    ...
    for i in range(num_windows):
        scores[i : i + window_len] += window_scores[
            i
...
def compute_accuracy(
    model, window_len, slugs, token_text, features, labels, num_to_test
):
    acc = 0.0
    for i in range(num_to_test):
        ...
        answer_text = correct_answer(
            features[doc_idx], labels[doc_idx], token_text[doc_idx]
        )
        print(
            f'{slugs[doc_idx]}: guessed "{predict_text}" with score {predict_score}, correct "{answer_text}"'
        )
...
class DocAccCallback(K.callbacks.Callback):
    def __init__(
        self, window_len, slugs, token_text, features, labels, num_to_test, logname
    ):
