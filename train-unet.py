...
for docrows in input_docs(max_docs=read_docs):
    ...
    labels.append([(0 if float(row['gross_amount']) <
                    target_thresh else 1) for row in docrows])
...
for doc_idx, doc_rows in enumerate(input_docs(max_docs=read_docs)):
    ...
    for pagenum, page in enumerate(pdf.pages):
        ...
        # Draw target tokens
        target_toks = [doc_rows[i]
                       for i in target_idx if doc_rows[i]['page'] == current_page]
