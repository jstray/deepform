...
def load_training_data_nocache(config):
    ...
    blank_token = {'token': '',         # for window padding
                   'x0':0, 
                   'y0':0, 
                   'x1':0, 
                   'y1':0, 
...
def load_training_data_from_files(
    ...
    if config.use_data_cache and os.path.isfile(pickle_destination):
        ...
        slugs, tokens, features, labels = pickle.load(
            open(pickle_destination, 'rb'))
    else:
        print('Loading training data...')
        slugs, tokens, features, labels = load_training_data_nocache(
            config)
        if config.use_data_cache:
            print('Saving training data to cache...')
            pickle.dump(
                (slugs, tokens, features, labels), open(
